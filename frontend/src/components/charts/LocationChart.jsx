...
export default function EnvironmentChart() {
  ...
  // const apiFetch = async () => await fetch('/api/hosts/groupby/location');
  // const apiJson = async (f) => await f.json();

  const apiFetch = async () => {
    const f = await fetch('/api/hosts/groupby/location');
  ...
  onMount(() => {
    ...
    Chart.defaults.font.family = "'Verdana', 'Geneva', 'sans-serif'";
  })
  ...
  const chartData = () => {
    return {
        labels: apiResource().json.result.map(function(i) { return i.location; }),
        datasets: [
            {
                label: 'Hosts',
