...
export default function HardwareChart() {
  ...
  // const apiFetch = async () => await fetch('/api/hosts/groupby/hardware');
  // const apiJson = async (f) => await f.json();
  const apiFetch = async () => {
    const f = await fetch('/api/hosts/groupby/hardware');
  ...
  var columns = [
    { accessorKey: 'product_name', header: 'Hardware', cell: v => v.getValue(), class: 'text-align-left', footer: 'Total' },
  ...
  const chartData = () => {
    return {
        labels: apiResource().json.result.map(function(i) { return i.product_name; }),
        datasets: [
            {
                label: 'Hosts',
