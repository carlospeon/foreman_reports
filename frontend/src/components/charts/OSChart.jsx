...
export default function OSChart() {
  ...
  // const apiFetch = async () => await fetch('/api/hosts/groupby/osmajor');
  // const apiJson = async (f) => await f.json();
  const apiFetch = async () => {
    const f = await fetch('/api/hosts/groupby/osmajor');
  ...
  var columns = [
    { accessorKey: 'osmajor', header: 'OS', cell: v => v.getValue(), class: 'text-align-left', footer: 'Total' },
  ...
  const chartData = () => {
    return {
        labels: apiResource().json.result.map(function(i) { return i.osmajor; }),
        datasets: [
            {
                label: 'Hosts',
