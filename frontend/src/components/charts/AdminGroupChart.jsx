...
export default function AdminGroupChart() {
  ...
  const apiFetch = async () => {
    const f = await fetch('/api/hosts/groupby/admingroup');
  ...
  var columns = [
    { accessorKey: 'admingroup', header: 'Admin Group', cell: v => v.getValue(), class: 'text-align-left', footer: 'Total' },
  ...
  const chartOptions = () => doughnutOptions('Hosts by Admin Group');

  const chartData = () => {
    return {
        labels: apiResource().json.result.map(function(i) { return i.admingroup; }),
        datasets: [
            {
                label: 'Hosts',
